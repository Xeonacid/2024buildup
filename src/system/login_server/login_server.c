...
int proc_login_login(void * sub_proc,void * recv_msg)
{
	...
	else
	{
		user_state=db_record->record;
        	...
        	Strncpy(passbuf,user_state->passwd,DIGEST_SIZE);
        	calculate_context_sm3(passbuf,DIGEST_SIZE,pass_digest);

		if(Memcmp(pass_digest,login_info->passwd,DIGEST_SIZE)==0)
		{	
