...
int modbus_server_read_message(BYTE * Buf, int len,void * sub_proc)
{
    ...
    */
    ret = modbus_output_datagram(Buf,len,sub_proc);
...
int modbus_output_datagram(BYTE * Buf, int len,void * sub_proc)
{
    int ret;
    int rc=0;
    int function_code;
    ...
    RECORD(MODBUS_STATE,CLIENT) * client_index;
    RECORD(MODBUS_STATE,SERVER) * server_index;
    BYTE databuf[DIGEST_SIZE*9];

    // build mbap struct
    mbap = Talloc0(sizeof(*mbap));
    if(mbap == NULL)
	    return -ENOMEM;
    
    Memcpy(mbap,Buf,sizeof(*mbap));

    if(mbap->length_lo +sizeof(*mbap) != len)
    {
	    print_cubeerr("modbus_channel: output datagram size error");
	    return -EINVAL;
    }
    ...
    if(datagram == NULL)
	    return -ENOMEM;
    void * datagram_template = memdb_get_template(TYPE_PAIR(MODBUS_TCP,DATAGRAM));
    if(datagram_template == NULL)
	    return -EINVAL;

    Buf[sizeof(*mbap)-1] -= 2;    

    data_len = blob_2_struct(Buf+sizeof(*mbap)-1,datagram,datagram_template);
    if(data_len<0)
	return data_len;

    send_msg=message_create(TYPE_PAIR(MODBUS_TCP,DATAGRAM),NULL);
    if(send_msg==NULL)
        return -EINVAL;

    message_add_record(send_msg,datagram);
    message_add_expand_data(send_msg,TYPE_PAIR(MODBUS_TCP,MBAP),mbap);
...
int modbus_channel_start(void * sub_proc,void * para)
{
    ...
    for (;;)
    {
        ...
        {
           ret=proc_send_modbus_cmd(sub_proc,recv_msg,subtype); 
        }
        else if((type==TYPE(MODBUS_TCP))&&(subtype == SUBTYPE(MODBUS_TCP,DATAGRAM)))
        {
           ret=proc_send_modbus_datagram(sub_proc,recv_msg); 
        }
...
int proc_send_modbus_cmd(void * sub_proc, void * recv_msg)
{
    ...
    *(UINT16 *)databuf = mbap->trans_id;
    ...
    print_cubeaudit("modbus_channel: send modbus cmd %d",ret); 
    return ret;
}
...
int proc_send_modbus_datagram(void * sub_proc, void * recv_msg)
{
    int ret;
    int len;
    int function_code;
    ...
    // get datagram from message

    ret = message_get_record(recv_msg,&datagram,0);
    if(ret<0)
	    return ret;
    ...
    // client(master) or server(slave)?
    if(mode==1)
    {
        // client(master)
        DB_RECORD * db_record;
        db_record = memdb_find_first(TYPE_PAIR(MODBUS_STATE,CLIENT),"unit_addr",&datagram->unit_id);
        if(db_record ==NULL)
		return -EINVAL;
        client_index = db_record->record;

	if(client_index -> state_machine == MODBUS_QUERY)
	{
		print_cubeaudit(" modbus_client: cmd after cmd, perhaps lost a command");
	}	
        client_index ->state_machine = MODBUS_QUERY;
    ...
    if(datagram_template == NULL)
	    return NULL;
    ret = struct_2_blob(datagram,databuf+sizeof(*mbap)-1,datagram_template);
    if(ret<0)
	    return ret;
    if(ret>254)
    {
	    print_cubeaudit("modbus_channel: too long send data");
	    return -EINVAL;
    }
    data_len=ret;
    mbap->length_hi=0;
    mbap->length_lo = data_len-1;
    Memcpy(databuf,mbap,sizeof(*mbap));
    if(mode == 0)	
    {
        server_index=ex_module_getpointer(sub_proc);
        channel_conn=getConnByUnitaddr(conn_hub,server_index->unit_addr);
        ret=channel_conn->conn_ops->write(channel_conn,databuf,sizeof(*mbap)+mbap->length_lo);
    }
    else if(mode == 1)
    {
        ret=client_conn->conn_ops->write(client_conn,databuf,sizeof(*mbap)+mbap->length_lo);
    }
    if(ret<0)
    {
	    if(mode == 1)
	   	print_cubeerr("modbus channel: master write data failed! unit_id %d no %d ",
				datagram->unit_id,mbap->trans_id );
	    else
	   	print_cubeerr("modbus channel: slave write data failed! unit_id %d no %d ",
				datagram->unit_id,mbap->trans_id );

    }
    else
    {
	    print_cubeaudit("modbus_channel: send modbus datagram %d ret %d",
            data_len,ret);
    }
    return ret;
}
